...
//
// 1. NOTHING IS SILENTLY LOST. LondonEventsManager hides a one-off that has
//    FINISHED from both tabs, so such a row inserts happily and is then
//    invisible everywhere — the admin never learns the email was parsed. So a
//    finished one-off is NEVER inserted; it comes back in `skipped` with its
//    date and a reason, and the admin decides (usually the model simply guessed
//    the wrong year). "Finished" means `end_date ?? date` is before today, not
//    `date` alone — see point 3. Every other rejection is reported the same way.
//
...
//
// 3. A RUN IS ONE ROW, NOT ONE DAY. The Gruffalo at the Lyric, a half-term
//    trail, a summer season — these run for weeks, and an email about them
//    names both ends. `date` is the FIRST day and `end_date` the LAST; a
//    genuine one-off leaves `end_date` null and behaves exactly as before.
//    `london_events.effective_end_date` is `coalesce(end_date, date)`, STORED
//    GENERATED by the database (migration 026), and is what every read side
//    filters on — so a run stays visible for its whole length instead of
//    disappearing the day after it opened. Being generated, it must NEVER
//    appear in an insert payload; write `end_date` and let Postgres do the
//    rest. An inverted range is never written: an end before the start is
...
- A ONE-OFF (a single dated event, e.g. "our summer fair is on 12 September"):
  set "date" to that date as YYYY-MM-DD, leave "end_date" null, and leave
  "weekday" null.
- A RUN (something on for a stretch of days, e.g. "13-31 August", "until 31
  August", "daily throughout the school holidays", "all summer"): set "date" to
  the FIRST day and "end_date" to the LAST day, both as YYYY-MM-DD. A show with
  a long run, a holiday programme or a season-long trail is ONE event with a
  start and an end — do not return one object per day.
- A WEEKLY REPEAT (e.g. "every Tuesday 10am", "Wednesdays in term time"):
  ...
  date in "start_date" as YYYY-MM-DD — do NOT put it in "date". If it says when
  the block ENDS, put that in "end_date".
- Leave "end_date" null whenever the email does not say when something ends.
  NEVER guess an end date, and never repeat the start date in it.
- If the email describes something as running across a season or the school
  holidays without naming dates ("Summer Screens, all summer", "Jazz & Soul
  through the holidays"), give the first and last day that period plainly
  covers in "date" and "end_date". If the email genuinely says nothing about
  when, set every date field to null rather than inventing one.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const rawEndDate = clean(entry.end_date, 40)
      ...
      let statedDate = parseIsoDate(rawDate)
      if (rawDate && !statedDate) {
        warnings.push(`date "${rawDate}" was not a usable calendar date and was ignored`)
      }

      // The last day of a RUN. Same strict parser as `date`, so a rollover like
      // 2026-02-31 is rejected here too rather than silently becoming 3 March.
      let statedEnd = parseIsoDate(rawEndDate)
      if (rawEndDate && !statedEnd) {
        warnings.push(`end date "${rawEndDate}" was not a usable calendar date and was ignored`)
      }

      // An end with no start ("open until 31 August", nothing about the opening
      // day) still describes something real, and dropping the event entirely is
      // the worse answer — so the end becomes the date. ONLY when no weekday
      // came back: "Wednesdays until 31 August" is a weekly block whose last
      // date is 31 August, and turning that into a one-off on 31 August would
      // silently lose the class.
      if (statedEnd && !statedDate && dow === null) {
        warnings.push(`email gave an end date (${statedEnd}) but no start date — listed as a single day on ${statedEnd}`)
        statedDate = statedEnd
        statedEnd = null
      }
      if (statedEnd && statedDate) {
        if (statedEnd < statedDate) {
          // An inverted range is never stored: `effective_end_date` would sit
          // before `date` and the row would read as finished before it began.
          warnings.push(`end date ${statedEnd} is before the start date ${statedDate} — the end date was ignored and this is listed as a single day`)
          statedEnd = null
        } else if (statedEnd === statedDate) {
          // A one-off, described twice. A redundant end helps nobody.
          statedEnd = null
        }
      }
      ...
      if (statedDate) {
        // THE VISIBILITY TRAP: a finished one-off inserts fine and is then
        // hidden on BOTH tabs. Report it instead of losing it.
        //
        // The test is when it FINISHES, not when it started. The Gruffalo
        // opening on 13 August and running to 31 August is on TODAY, the 16th,
        // and belongs in What's On — skipping it because its first day is
        // behind us is exactly the bug this end_date work exists to fix.
        const finishes = statedEnd ?? statedDate
        if (finishes < today) {
          skipped.push({
            title,
            date: statedDate,
            reason: statedEnd
              ? `This run finished on ${statedEnd} (it ran from ${statedDate}; today is ${today}) — it would be invisible in the Pending tab, so it was not added. Check the year.`
              : `Date has already passed (today is ${today}) — it would be invisible in the Pending tab, so it was not added. Check the year.`,
          })
          continue
        }
        date = statedDate
        ...
        // `date` here is the NEXT occurrence, not the block's first day, so a
        // stated end can easily sit behind it (a block that finished, or one
        // whose last session is before next week's). Carry the end only when it
        // is still ahead of the row's own date — never write a range that ends
        // before it starts, and never change what a regular does today.
        if (statedEnd) {
          if (statedEnd >= date) {
            endDate = statedEnd
          } else {
            warnings.push(`the block's last date (${statedEnd}) is before the first listing (${date}) — end date left blank; check whether this regular has finished`)
          }
        }
      } else {
        skipped.push({
      ...
      candidates.push({
        ...
        row: {
          title,
          date,
          // Null for a one-off. `effective_end_date` is generated from this by
          // the database and is deliberately NOT written here.
          end_date: endDate,
          time,
          venue,
          location,
    ...
    return new Response(
      JSON.stringify({
        ...
        events: inserted.map((c) => ({
          title: c.row.title,
          date: c.row.date,
          // Null unless it is a run; the admin banner shows a range when set.
          end_date: c.row.end_date,
