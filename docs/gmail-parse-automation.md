...
| `whats-on/error` label appears | The call failed — the summary email names the reason | **Read the summary email first.** If it says *FAILED — nothing was added*, fix the cause below and swap `whats-on/error` back to `whats-on` to retry. If it says **OUTCOME UNKNOWN**, do NOT re-label until you have checked Pending — see the next row |
| Summary email says **OUTCOME UNKNOWN — READ THIS ONE** | The request went out but the answer never came back (a timeout, or a 502/503/504 from the platform rather than the parser). The events may or may not have been created — from outside there is no way to tell | **Check `/admin/whats-on` → Pending for that organiser's events before doing anything.** If they are there, just remove the `whats-on/error` label and you are done. If they are not, re-label with `whats-on` to run it again. Re-labelling without looking is the one action that creates duplicates |
...
| `OpenAI API error` / `Failed to parse the OpenAI response as JSON` | Model or OpenAI outage, or `OPENAI_API_KEY` unset on the function | Safe to re-label and retry — these all happen *before* anything is written, so nothing was added. If it repeats, check the function's `OPENAI_API_KEY` secret in the Supabase dashboard |
